...
static const char* hm_set_entry(entries_in_hm* entries, size_t capacity, const char* key, void* value, size_t* plen) {
    ...
    while (entries[idx].key != NULL) {
        ...
        // This is the linear probing part. Hash has collisioned so put it into next one index, when the loops
        // come back, the hash is checked again. If no match the index is increased by one over until one is found empty
        idx++;
